...
function Bar({ value, max, color }) {
  return (
    <div style={{ height:3, background:"rgba(255,255,255,.05)", borderRadius:2, overflow:"hidden" }}>
      <div style={{ width:`${Math.min(100, Math.round(value/max*100))}%`, height:"100%", background:color, borderRadius:2, transition:"width .6s ease" }} />
    </div>
  );
}
...
const equityCurveData = [
  { day:1, equity:10000 }, { day:5, equity:10052 }, { day:10, equity:10098 },
  { day:15, equity:10145 }, { day:20, equity:10087 }, { day:25, equity:10162 },
  { day:30, equity:10208 }, { day:35, equity:10154 }, { day:40, equity:10223 },
  { day:45, equity:10287 }, { day:50, equity:10341 }, { day:55, equity:10296 },
  { day:60, equity:10352 }, { day:65, equity:10388 }, { day:70, equity:10427 },
];

function EquityCurve({ data, width = 900, height = 200 }) {
  ...
  const points = data.map((d, i) => `${toX(i)},${toY(d.equity)}`).join(" ");
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${width} ${height}`} style={{ display:"block" }}>
      ...
      {[0, 0.25, 0.5, 0.75, 1].map((frac) => {
        ...
        return (
          <g key={frac}>
            <line x1={pad.left} y1={y} x2={width - pad.right} y2={y} stroke={T.border} strokeWidth="0.5" />
            <text x={pad.left - 6} y={y + 3} textAnchor="end" fontSize={8} fill={T.txtDim}>{val.toFixed(0)}</text>
          </g>
        );
      })}
      {/* Area fill */}
      ...
      {/* Data dots */}
      {data.map((d, i) => (
        <circle key={i} cx={toX(i)} cy={toY(d.equity)} r="3" fill={T.card} stroke={lineColor} strokeWidth="1.5" />
      ))}
    </svg>
  );
}
...
function AgentRow({ agent, expanded, onToggle, i }) {
  ...
  return (
    <div className={`af-agent${expanded ? " open" : ""}`} style={{ animationDelay:`${i * 0.03}s` }} onClick={onToggle}>
                      ...
                      return (
                        <div key={d.id}>
                          ...
                          <div style={{ height:4, background:"rgba(255,255,255,.05)", borderRadius:2 }}>
                            <div style={{ width:`${pct}%`, height:"100%", background:d.color, borderRadius:2, transition:"width .7s ease" }} />
                          </div>
                        </div>
                      );
                    })}
        ...
        {tab === "performance" && (
          <div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:10, marginBottom:18 }}>
              {[
                {l:"Win Rate", v:`${ACCOUNT.winRate}%`, sub:`${ACCOUNT.totalTrades} trades`, col:T.green},
                {l:"Profit Factor", v:ACCOUNT.profitFactor.toFixed(2), sub:"gross profit / gross loss", col:T.blue},
                {l:"Sharpe Ratio", v:ACCOUNT.sharpeRatio.toFixed(2), sub:"risk-adjusted return", col:T.purple},
                {l:"Best Session", v:ACCOUNT.bestSession, sub:"London Open", col:T.gold},
                {l:"Max Drawdown", v:`-${ACCOUNT.drawdown}%`, sub:"intraday peak-to-trough", col:T.red},
              ].map(m => (
                <div key={m.l} style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:8, padding:"14px 14px" }}>
                  <div style={{ fontSize:10, color:T.txtDim, marginBottom:4 }}>{m.l}</div>
                  <div style={{ fontFamily:T.mono, fontSize:22, fontWeight:600, color:m.col }}>{m.v}</div>
                  <div style={{ fontSize:9, color:T.txtDim, marginTop:3 }}>{m.sub}</div>
                </div>
              ))}
            </div>

            {/* Equity Curve */}
            <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:10, padding:"18px 20px", marginBottom:18 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
                <div>
                  <div style={{ fontSize:13, fontWeight:500, color:T.txt }}>Equity Curve</div>
                  <div style={{ fontSize:10, color:T.txtDim, marginTop:2 }}>Daily cumulative P&L — 2024</div>
                </div>
                <div style={{ display:"flex", gap:8 }}>
                  <span style={{ fontSize:10, color:T.green }}>● Real</span>
                  <span style={{ fontSize:10, color:T.gold }}>● Simulated</span>
                </div>
              </div>
              <EquityCurve width={900} height={200} data={equityCurveData} />
            </div>

            {/* Weekly Performance */}
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:18 }}>
              <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:10, padding:"18px 20px" }}>
                <div style={{ fontSize:13, fontWeight:500, color:T.txt, marginBottom:14 }}>Daily Performance</div>
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                  {WEEKLY_PERF.map(d => (
                    <div key={d.day} style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                      <span style={{ fontSize:11, color:T.txtMid, width:70 }}>{d.day}</span>
                      <div style={{ flex:1, height:6, background:T.surface, borderRadius:3, overflow:"hidden", margin:"0 10px" }}>
                        <div style={{
                          width:`${Math.min(Math.abs(d.pips) / 100 * 100, 100)}%`,
                          height:"100%",
                          background: d.pips >= 0 ? T.green : T.red,
                          borderRadius:3,
                          marginLeft: d.pips < 0 ? "auto" : 0,
                        }} />
                      </div>
                      <span style={{ fontFamily:T.mono, fontSize:11, color:d.pips >= 0 ? T.green : T.red, width:45, textAlign:"right" }}>
                        {d.pips >= 0 ? "+" : ""}{d.pips}p
                      </span>
                      <span style={{ fontSize:9, color:T.txtDim, width:30, textAlign:"right" }}>{d.winRate}%</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:10, padding:"18px 20px" }}>
                <div style={{ fontSize:13, fontWeight:500, color:T.txt, marginBottom:14 }}>Session Performance</div>
                <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                  {KILL_ZONES.filter(k => k.profitPips > 0 || k.trades > 0).map(k => (
                    <div key={k.name} style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                      <div>
                        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                          ...
                          <span style={{ fontSize:11, color:T.txtMid }}>{k.name}</span>
                        </div>
                        <div style={{ fontSize:9, color:T.txtDim, marginTop:2, marginLeft:12 }}>{k.trades} trades · {k.winRate}% WR</div>
                      </div>
                      <div style={{ textAlign:"right" }}>
                        <div style={{ fontFamily:T.mono, fontSize:13, color:T.green }}>
                          +{k.profitPips}p
                        </div>
                        <div style={{ fontSize:9, color:T.txtDim }}>total</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {/* Backtest Summary */}
            <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:10, padding:"18px 20px" }}>
              <div style={{ fontSize:13, fontWeight:500, color:T.txt, marginBottom:14 }}>Strategy Backtests</div>
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                <thead>
                  <tr>
                    {["Strategy","Symbol","Period","Win Rate","Profit Factor","Sharpe","Max DD","Return"].map(h => (
                      <th key={h} style={{ fontSize:9, color:T.txtDim, fontWeight:400, textAlign:"left", paddingBottom:8, borderBottom:`1px solid ${T.border}` }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {BACKTEST_RESULTS.map(b => (
                    <tr key={b.strat}>
                      <td style={{ padding:"7px 0", fontSize:11, color:T.gold }}>{b.strat}</td>
                      <td style={{ padding:"7px 0", fontSize:11, color:T.txtMid }}>{b.symbol} {b.tf}</td>
                      <td style={{ padding:"7px 0", fontSize:10, color:T.txtDim }}>{b.period.split(" ")[0]}</td>
                      <td style={{ padding:"7px 0", fontSize:11, color:T.green }}>{b.winRate}%</td>
                      <td style={{ padding:"7px 0", fontSize:11, color:T.blue }}>{b.pf}</td>
                      <td style={{ padding:"7px 0", fontSize:11, color:T.purple }}>{b.sharpe}</td>
                      <td style={{ padding:"7px 0", fontSize:11, color:T.red }}>{b.mdd}%</td>
                      <td style={{ padding:"7px 0", fontSize:11, color:T.green }}>+{b.ret}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
